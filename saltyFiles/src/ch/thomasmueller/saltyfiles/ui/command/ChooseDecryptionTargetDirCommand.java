 ...
 *  
 */
public class ChooseDecryptionTargetDirCommand implements Command
{
	...
	public void doCommand(ActionEvent event)
	{
		...
		if (file != null)
		{
			DataField decTargetDirDataField = DataModel.getInstance()
					.getDataField(DataModel.DECRYPT_TARGET_DIR_STRING);
			if (decTargetDirDataField == null)
			{
				throw new RuntimeException("DataField not registered " 
						+ DataModel.DECRYPT_TARGET_DIR_STRING);			
			}
			else
			{
				decTargetDirDataField.setValue(file.getAbsolutePath());
			}
		}
		else
		{
